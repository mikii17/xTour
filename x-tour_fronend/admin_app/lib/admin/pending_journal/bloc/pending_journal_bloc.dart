...
class PendingJournalBloc extends Bloc<PendingJournalEvent, PendingJournalState> {
  final PendingJournalRepository pendingJournalRepository;
  PendingJournalBloc({required this.pendingJournalRepository}) : super(PendingJournalInitial()) {
  ...
  FutureOr<void> loadJournal(LoadPendingJournal event, Emitter<PendingJournalState> emit) async {
    ...
    try {
      Map<String, dynamic> query = {
        ...
        final List<Journal> journals = await this.pendingJournalRepository.getPendingJournal(query);
  ...
  FutureOr<void> loadMoreJournal(LoadMorePendingJournal event, Emitter<PendingJournalState> emit) async {
  ...
  FutureOr<void> approveJournal(ApprovePendingJournal event, Emitter<PendingJournalState> emit) async {
      final journals = (state as PendingJournalLoaded).journals;
    try {
      await pendingJournalRepository.approvePendingJournal(event.id, event.journal);
  ...
  FutureOr<void> deleteJournal(DeletePendingJournal event, Emitter<PendingJournalState> emit) async {
      final journals = (state as PendingJournalLoaded).journals;
    try {
