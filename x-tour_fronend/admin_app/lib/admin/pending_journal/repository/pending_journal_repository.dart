...
class PendingJournalRepository {
  ...
  const PendingJournalRepository({required this.pendingJournalService, required this.authBloc});
   
   Future<List<Journal>> getPendingJournal(Map<String, dynamic> query) async {
    final response = await pendingJournalService.getPendingJournals(query: query, accesstoken: getAccessHeader());
   ...
   Future<Journal> approvePendingJournal(String id, Journal journal) async {
    final response = await pendingJournalService.approvePendingJournal(id:id, journal: journal, accesstoken: getAccessHeader());
    if (response.isSuccessful){
      return response.body!;
    }
    if(response.statusCode == HttpStatus.unauthorized){
