...
class PendingPostRepository {
  ...
  const PendingPostRepository({required this.pendingPostService, required this.authBloc});
   
   Future<List<Posts>> getPendingPosts(Map<String, dynamic> query) async {
    final response = await pendingPostService.getPendingPosts(query: query, accesstoken: getAccessHeader());
   ...
   Future<Posts> approvePendingPosts(String id, Posts post) async {
    final response = await pendingPostService.approvePendingPost(id:id, post: post, accesstoken: getAccessHeader());
    if (response.isSuccessful){
      return response.body!;
    }
