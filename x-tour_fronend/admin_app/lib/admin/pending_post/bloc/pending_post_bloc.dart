...
class PendingPostBloc extends Bloc<PendingPostEvent, PendingPostState> {
  ...
  PendingPostBloc({required this.pendingPostRepository}) : super(PendingPostInitial()) {
  ...
  FutureOr<void> loadPost(LoadPendingPost event, Emitter<PendingPostState> emit) async {
    ...
    try {
      ...
      final List<Posts> posts = await pendingPostRepository.getPendingPosts(query);
  ...
  FutureOr<void> loadMorePost(LoadMorePendingPost event, Emitter<PendingPostState> emit) async {
  ...
  FutureOr<void> approvePost(ApprovePendingPost event, Emitter<PendingPostState> emit) async {
  ...
  FutureOr<void> deletePost(DeletePendingPost event, Emitter<PendingPostState> emit) async {
   final posts = (state as PendingPostLoaded).posts;
    try {
