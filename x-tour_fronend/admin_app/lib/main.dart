...
void main() {
  final chopperClient = ChopperClient(
      ...
      services: [
        PendingPostService .create(),
        PendingJournalService.create()
      ],
  ...
  final chopperClient2 = ChopperClient(
   ...
   PendingPostService  pendingPostService =  PendingPostService.create(chopperClient);
   PendingJournalService  pendingJournalService =  PendingJournalService.create(chopperClient);
  ...
  LoginBloc loginBloc = LoginBloc(authBloc: authBloc, authRepository: authRepository);

  PendingJournalRepository pendingJournalRepository=PendingJournalRepository(pendingJournalService: pendingJournalService, authBloc: authBloc);
  PendingPostRepository pendingPostRepository=PendingPostRepository(pendingPostService: pendingPostService, authBloc: authBloc);
  ...
  runApp( MultiBlocProvider(
    ...
    child: MultiRepositoryProvider(providers: [
      RepositoryProvider.value(value: pendingPostRepository,),
      RepositoryProvider.value(value: pendingJournalRepository),
    ], child: AppRoute(authBloc: authBloc,)),
  ) 
       ,
  );
}
